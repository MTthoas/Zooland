    ...
    try {
      ...
      if (error instanceof Error) {
        res
          .status(500)
          .json({
            message: `Erreur lors de la récupération des statistiques quotidiennes: ${error.message}`,
          });
      } else {
        res
          .status(500)
    ...
    try {
      ...
      if (error instanceof Error) {
        res
          .status(500)
          .json({
            message: `Erreur lors de la récupération des statistiques hebdomadaires: ${error.message}`,
          });
      } else {
        res
          .status(500)
    ...
    try {
      ...
      if (error instanceof Error) {
        res
          .status(500)
          .json({
            message: `Erreur lors de la récupération des statistiques en direct : ${error.message}`,
          });
      } else {
        res
          .status(500)
          .json({
            message:
              "Erreur lors de la récupération des statistiques en direct.",
          });
      }
    }
  }
    ...
    try {
      ...
      if (error instanceof Error) {
        res
          .status(500)
          .json({
            message: `Erreur lors de la suppression de toutes les statistiques: ${error.message}`,
          });
      } else {
        res
          .status(500)
          .json({
            message:
              "Erreur lors de la suppression de toutes les statistiques.",
          });
      }
    }
  }
  ...
  async suppStatByIdSpace(req: Request, res: Response) {
    try {
      ...
      res
        .status(200)
        .json({
          message: `Les statistiques pour l'espace avec l'ID ${spaceId} ont été supprimées avec succès.`,
        });
    } catch (error) {
      if (error instanceof Error) {
        res
          .status(500)
          .json({
            message: `Erreur lors de la suppression des statistiques pour l'espace avec l'ID ${spaceId}: ${error.message}`,
          });
      } else {
        res
          .status(500)
          .json({
            message: `Erreur lors de la suppression des statistiques pour l'espace avec l'ID ${spaceId}.`,
          });
      }
    }
  }
